...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Cultures.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Cultures.Status.DRAFT)
...
class Cultures(models.Model): 
    ...
    likes = models.ManyToManyField(User, related_name='liked_cultural_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation')
    section_name = models.CharField(max_length=50, default='Culture', editable=False) 
    ...
    def get_absolute_url(self):
        return reverse('cultures:post_detail',
    ...
    def get_comments(self):
        return self.culture_comments.filter(culture_parent=None).filter(active=True)
    
class Cultures_Comment(models.Model):
    User = get_user_model()     
    post = models.ForeignKey(Cultures,                            
                             on_delete=models.CASCADE,                             
                             related_name='culture_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='cultureuser_comments' )     
    ...
    culture_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_cultural_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_cultural_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Culture Comment', editable=False) 
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Cultures_Comment.objects.filter(culture_parent=self).filter(active=True)
