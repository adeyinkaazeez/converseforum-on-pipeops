...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Educations.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Educations.Status.DRAFT)
...
class Educations(models.Model): 
    ...
    section_name = models.CharField(max_length=50, default='Education', editable=False) 
    ...
    def get_absolute_url(self):
        return reverse('educations:post_detail',
    ...
    def get_comments(self):
        return self.education_comments.filter(education_parent=None).filter(active=True)
    

class Educations_Comment(models.Model):
    User = get_user_model()     
    post = models.ForeignKey(Educations,                            
                             on_delete=models.CASCADE,                             
                             related_name='education_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='educationuser_comments' )    
    ...
    education_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_education_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_education_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Education Comment', editable=False) 
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Educations_Comment.objects.filter(education_parent=self).filter(active=True)
