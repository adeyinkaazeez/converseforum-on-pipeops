...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Campuss.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Campuss.Status.DRAFT)
...
class Campuss(models.Model): 
    ...
    likes = models.ManyToManyField(User, related_name='liked_campus_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation')  
    section_name = models.CharField(max_length=50, default='Campus', editable=False)
    ...
    def get_absolute_url(self):
        return reverse('campuss:post_detail',
    ...
    def get_comments(self):
        return self.campus_comments.filter(c_parent=None).filter(active=True)
    
class Campuss_Comment(models.Model): 
    User = get_user_model()   
    post = models.ForeignKey(Campuss,                            
                             on_delete=models.CASCADE,                             
                             related_name='campus_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='campususer_comments' )    
    ...
    c_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="c_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_campus_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_campus_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Campus Comment ', editable=False)
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Campuss_Comment.objects.filter(c_parent=self).filter(active=True)
