...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Celeb.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Celeb.Status.DRAFT)
...
class Celeb(models.Model): 
    ...
    likes = models.ManyToManyField(User, related_name='liked_celeb_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation')
    section_name = models.CharField(max_length=50, default='Celebrity', editable=False)                                         
    ...
    def get_absolute_url(self):
        return reverse('celeb:post_detail',
    ...
    def get_comments(self):
        return self.celebrity_comments.filter(celeb_parent=None).filter(active=True)
    
class Celeb_Comment(models.Model): 
    User = get_user_model()   
    post = models.ForeignKey(Celeb,                            
                             on_delete=models.CASCADE,                             
                             related_name='celebrity_comments') 
    
    name = models.ForeignKey(User,                             
                                 on_delete=models.CASCADE,
                                 related_name='celebususer_comments' )                       
    ...
    celeb_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="celeb_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_celeb_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_celeb_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Celebrity Comment', editable=False) 
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Celeb_Comment.objects.filter(celeb_parent=self).filter(active=True)
