...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Business_Articles.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Business_Articles.Status.DRAFT)
    

class Business_Articles(models.Model): 
    User = get_user_model()
    ...
    author = models.ForeignKey(User,                             
                                ...
                                related_name='business_article_posts' )
    body = RichTextField()
    ...
    likes = models.ManyToManyField(User, related_name='liked_business_article_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation') 
    section_name = models.CharField(max_length=50, default='Business_Articles', editable=False)
    ...
    def get_absolute_url(self):
        return reverse('business__articles:post_detail',
    ...
    def get_comments(self):
        return self.business_article_comments.filter(b_parent=None).filter(active=True)
    
class Business_Articles_Comment(models.Model): 
    User = get_user_model()   
    post = models.ForeignKey(Business_Articles,                            
                             on_delete=models.CASCADE,                             
                             related_name='business_article_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='business_articleuser_comments' )    
    ...
    b_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="breplies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_business_article_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_business_article_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Business_Articles_Comment', editable=False)
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Business_Articles_Comment.objects.filter(b_parent=self).filter(active=True)
