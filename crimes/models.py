...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Crimes.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Crimes.Status.DRAFT)
...
class Crimes(models.Model): 
    ...
    likes = models.ManyToManyField(User, related_name='liked_crime_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation')
    section_name = models.CharField(max_length=50, default='Crime', editable=False) 
    ...
    def get_absolute_url(self):
        return reverse('crimes:post_detail',
    ...
    def get_comments(self):
        return self.crime_comments.filter(crime_parent=None).filter(active=True)
    
    

class Crimes_News_Comment(models.Model):  
    User = get_user_model()     
    post = models.ForeignKey(Crimes,                            
                             on_delete=models.CASCADE,                             
                             related_name='crime_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='crimeuser_comments' )    
    ...
    crime_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="crime_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_crime_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_crime_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Crime Comment', editable=False)
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Crimes_News_Comment.objects.filter(crime_parent=self).filter(active=True)
